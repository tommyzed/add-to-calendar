    ...
    try {
        ...
        if (typeof err === 'object' && err !== null && 'result' in err) {
            ...
            if (errObj.result?.error?.code === 401) {
                try {
                    console.log("Token possibly expired, attempting refresh and retry...");
                    await refreshAccessToken();
                    const retryRequest = gapi.client.calendar.events.insert({
                        'calendarId': 'primary',
                        'resource': event as gapi.client.calendar.Event,
                    });
                    const retryResponse = await retryRequest;
                    return retryResponse.result;
                } catch (retryErr: unknown) {
                    console.warn("Retry failed after refresh:", retryErr);
